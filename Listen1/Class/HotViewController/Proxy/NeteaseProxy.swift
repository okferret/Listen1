...
extension NeteaseProxy {
    ...
    internal static func recommends(offset: Int, length: Int, cleanAll: Bool) -> Single<[Playlist.Snapshot]> {
        return NeteaseProvider.shared.rx.request(.recommends(offset, length)).retry(2).observeOn(ConcurrentDispatchQueueScheduler.init(queue: .global())).flatMap { (resp) -> Single<[Playlist.Snapshot]> in
